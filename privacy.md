...
| What | Where |
| --- | --- |
| Medicines, strengths, quantities, expiry dates, labels, your notes | Encrypted app storage |
| Courses, schedules and the doses you have logged | Encrypted app storage |
| Photographs of packaging and patient leaflets | Encrypted files in the app's private storage |
| Reminder settings and the in-app reminder history | Encrypted app storage |
...
- **Camera** — to photograph packaging or a leaflet.
- **Photo library** — to attach a picture you already have, and, separately, "add only" permission to
  save one of the app's photos back to your library when you ask it to.

Photos taken or chosen this way are stored in the app's own encrypted storage. They are not uploaded.
...
Reminders are scheduled locally by your iPhone. The app has no push-notification server — it is not
technically capable of sending you anything from outside your device. What a reminder says on your
lock screen is up to you: by default it does **not** name the medicine.
...
Remedies is not directed at children. It is a household organiser, and the information in it is
entered by the adult using the app.
...
If the app ever starts collecting anything — for example if optional cloud sync is added — this
policy will be updated **before** that ships, and the change will be visible in the app. The date at
the top of this document is when it last changed.
